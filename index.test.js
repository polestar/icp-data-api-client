...
import * as dataApiClient from './index';
...
dataApiClient.__get__ = (fnName) => dataApiClient[fnName];
...
describe('[dataApiClient]', () => {
  ...
  describe('query configuration parsing', () => {
    test('mergeConfig', async () => {
      ...
      let result = mergeConfig(
        { secretArn: 'secretArn', resourceArn: 'resourceArn' },
        { database: 'db' }
      );
      expect(result).toEqual({
        secretArn: 'secretArn',
    ...
    describe('parseHydrate', () => {
      ...
      test('parseHydrate - from args, not boolean (error)', async () => {
        let result = () =>
          parseHydrate({ hydrateColumnNames: true }, [{ hydrateColumnNames: 'false' }]);
  ...
  describe('query parameter processing', () => {
    ...
    describe('processParams', () => {
      ...
      test('typecasting params', async () => {
        ...
        expect(escapedSql).toBe(
          'INSERT INTO users(id, name, meta) VALUES(:id::uuid, :name, :meta::jsonb)'
        );
        expect(processedParams).toEqual([
  ...
  describe('querying', () => {
    describe('query', () => {
      ...
      const mockRDSSend = jest.fn().mockReturnValue(require('./test/sample-query-response.json'));
      let parameters = {};
      ...
      test('simple query', async () => {
        ...
        expect(mockRDSSend).toHaveBeenCalledWith({
          args: [
            {
              database: 'db',
        ...
        expect(result).toEqual({
          records: [
            [
              ...
              'Category 1',
              null,
              new Date('2019-11-12T22:00:11.000Z'),
              new Date('2019-11-12T22:15:25.000Z'),
              null
            ],
            [
              2,
        ...
        expect(result).toEqual({
          records: [
            [
              ...
              'Category 1',
              null,
              new Date('2019-11-12 22:00:11'),
              new Date('2019-11-12 22:15:25'),
              null
            ],
            [
              2,
